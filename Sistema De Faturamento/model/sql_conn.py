...
def fetch_faturamento_data(conn, ano, itens_permitidos=None):
    ...
    base_query = """SELECT fb.id_itens_grupo_receita AS fb_id_itens_grupo_receita, 
    ...
    params = [ano]
    placeholders = ",".join(["?"] * len(itens_permitidos))
    base_query += f" AND dgr.id_itens_grupo_receita IN ({placeholders})"
    params.extend(itens_permitidos)
...
def fetch_clientereceita_data(conn, ano, itens_permitidos=None):
    if not itens_permitidos:
        st.warning("Nenhum item permitido foi encontrado para o usuário.")
        return pd.DataFrame()   
   
    placholders = ",".join(["?"] * len(itens_permitidos))
    params = [ano] + itens_permitidos
    query = f"""WITH RankedClientes AS (
               SELECT cl.nm_Cliente, dgr.id_itens_grupo_receita,
                ...
                JOIN *******DW.DW.DItensGrupoReceita dgr  
                    ON fb.id_itens_grupo_receita = dgr.id_itens_grupo_receita
                WHERE YEAR(ddl.dt_lancamento) = ? AND dgr.id_itens_grupo_receita IN ({placholders})
...
def fetch_mensalfaturamento_data(conn, ano, itens_permitidos=None):
    if not itens_permitidos:
        st.warning("Nenhum item permitido foi encontrado para o usuário.")
        return pd.DataFrame()
    ...
    params = [ano]
    placholders = ",".join(["?"] * len(itens_permitidos))
    query += f" AND dgr.id_itens_grupo_receita IN ({placholders})"
    params.extend(itens_permitidos)
    query+="""GROUP BY dgr.nm_itens_grupo_receita, DATENAME(MONTH, ddl.dt_lancamento), MONTH(ddl.dt_lancamento)
            HAVING SUM(fb.vl_total_nf) > 0
...
def fetch_receitaliquidaAtual_data(conn, ano, itens_permitidos=None):
    if not itens_permitidos:
        st.warning("Nenhum item permitido foi encontrado para o usuário.")
        return pd.DataFrame()
    
    placholders = ",".join(["?"] * len(itens_permitidos))
    params = [ano, ano, ano, ano, ano] + itens_permitidos   

    # Calcular a receita liquida do ano de cada grupo de receitas  
    query =f"""SELECT fb.id_itens_grupo_receita AS fb_id_itens_grupo_receita, dgr.nm_itens_grupo_receita AS nm_itens_grupo_receita, 
...
def fetch_diariofaturamento_data(conn, ano, itens_permitidos=None):
    # Verificar se a lista de itens permitidos está vazia
    if not itens_permitidos or len(itens_permitidos) == 0:
        st.warning("Nenhum item permitido foi encontrado para o usuário.")
        return pd.DataFrame()

    #Busca dados de faturamento diário com filtro por itens permitidos      
    base_query = """SELECT dgr.id_itens_grupo_receita, dgr.nm_itens_grupo_receita,
                      ...
                      WHERE YEAR(ddl.dt_lancamento) = ?"""   
    params = [ano]
    # Adiciona filtro por itens permitidos (IDs)
    placeholders = ",".join(["?"] * len(itens_permitidos))
    base_query += f" AND dgr.id_itens_grupo_receita IN ({placeholders})"
    params.extend(itens_permitidos)
   # Finalizar a consulta   
    full_query = base_query + """ GROUP BY ddl.dt_lancamento, dgr.nm_itens_grupo_receita, dgr.id_itens_grupo_receita
                                  HAVING SUM(fb.vl_total_nf) > 0
                                  ORDER BY dgr.nm_itens_grupo_receita, ddl.dt_lancamento, dgr.id_itens_grupo_receita;"""
...
def fetch_diarioportugues_data(conn, ano):
    # Lista de Grupo de Receita por Dia em ano com data formatada em português
