...
from model.sql_conn import create_connection, fetch_clientes_data, fetch_clienteAtualAnter_data
from model.database import get_group_name    

st.set_page_config(page_title="10 Top Clientes", page_icon=":🏅")
...
if conn:
    if 'ano' not in st.session_state:
        ...
        if dados_atual is not None:
            ...
            delta_faturamento = 0
            delta_porcentual = 0
        st.header("Tops 10 dos Clientes & Comparação do Ano")
        st.divider()
        ...
        with col1:
            ...
            if dados_anter is not None and dados_anter['subtotal'] != 0:
                st.metric(
                    ...
                    #value=f"R$ {locale.format_string('%.0f', dados_atual['subtotal'], grouping=True)}" if dados_atual is not None else "R$ 0",
                    #delta=f"R$ {locale.format_string('%.0f', delta_faturamento, grouping=True)}",
        ...
        # Guia de tabela
        with guia_tabela:
            # Mostrar a Tabela dos Clientes
            st.subheader(f" 🥇 Listagem dos 10 Tops Clientes em {ano_selecionado}")
            st.dataframe(
